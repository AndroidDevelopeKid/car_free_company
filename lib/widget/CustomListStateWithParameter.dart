...
mixin CustomListStateWithParameter<T extends StatefulWidget> on State<T>, AutomaticKeepAliveClientMixin<T>{
  ...
  @protected
  Future<Null> handleRefresh(driverIdNumber, driverName, phoneNumber, vehicleCode, skipCountInit) async {
    if (isLoading) {
      return null;
    }
    isLoading = true;
    page = 1;
    var res = await requestRefresh(driverIdNumber, driverName, phoneNumber, vehicleCode, skipCountInit);
    resolveRefreshResult(res);
  ...
  ///下拉刷新数据
  @protected
  requestRefresh(driverIdNumber, driverName, phoneNumber, vehicleCode, skipCountInit) async {}
